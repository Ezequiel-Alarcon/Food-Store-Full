...
class ProductoService(base_service[Producto, ProductoCreate, ProductoUpdate, ProductoUnitOfWork]):
    ...
    def _validar_nombre_unico(self, name: str, exclude_id: Optional[int] = None) -> None:
        ...
        if producto and producto.id != exclude_id:
            raise HTTPException(status_code=400, detail=f"El nombre '{name}' ya está en uso por otro producto")


    def _to_read_full(self, producto: Producto) -> ProductoReadFull:
        categoria_links = self.uow.producto_categorias.list_by_producto(cast(int, producto.id))
        ...
        with self.uow:
            ...
            for i, cat_id in enumerate(data.categoria_ids):
                ...
                if not cat or getattr(cat, "deleted_at", None) is not None:
                    raise HTTPException(status_code=404, detail=f"Categoría con ID {cat_id} no encontrada o eliminada")
            ...
            if data.ingrediente_ids:
                for ing_id in data.ingrediente_ids:
                    ...
                    if not ing or getattr(ing, "deleted_at", None) is not None:
                        raise HTTPException(status_code=404, detail=f"Ingrediente con ID {ing_id} no encontrado o eliminado")
    ...
    def update(self, producto_id: int, data: ProductoUpdate) -> ProductoReadFull:
        with self.uow:
            ...
            if data.categoria_ids is not None:
                viejas_cats = self.uow.producto_categorias.list_by_producto(producto_id)
                ...
                for i, cat_id in enumerate(data.categoria_ids):
                    ...
                    link = ProductoCategoria(producto_id=producto_id, categoria_id=cat_id, es_principal=(i == 0))
                    self.uow.producto_categorias.add(link)
                    
            if data.ingrediente_ids is not None:
                viejos_ings = self.uow.producto_ingredientes.list_by_producto(producto_id)
                ...
                for ing_id in data.ingrediente_ids:
                    ...
                    link_ing = ProductoIngrediente(producto_id=producto_id, ingrediente_id=ing_id)
                    self.uow.producto_ingredientes.add(link_ing)
